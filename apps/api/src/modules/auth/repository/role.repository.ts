...
export const roleRepository = {
  ...
  listByOrg: async (organizationId: string) => {
    ...
    for (const row of rows) {
      ...
      // ⚡ Bolt: Cache Map lookup to avoid redundant search operations (O(1) instead of 2x O(1))
  ...
  findByIdWithPermissions: async (organizationId: string, roleId: string) => {
    ...
    // ⚡ Bolt: Use a single loop to avoid intermediate array allocations and double traversals
    const permissions = [];
    for (const row of rows) {
      if (row.permission) {
