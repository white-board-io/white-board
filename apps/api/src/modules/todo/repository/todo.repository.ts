...
export const todoRepository = {
  ...
  delete: async (id: string): Promise<Todo | undefined> => {
    // ⚡ Bolt: Use .returning() to atomically perform the mutation and check for existence
  ...
  toggle: async (id: string): Promise<Todo | undefined> => {
    // ⚡ Bolt: Atomically toggle the boolean status without checking existence first
    const results = await db
      .update(todos)
