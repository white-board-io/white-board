...
import { db, eq, not, desc, sql } from "@repo/database";
...
export const todoRepository = {
  ...
  delete: async (id: string): Promise<Todo | undefined> => {
    // ⚡ Bolt: Optimized to return full record using .returning() to avoid prior findById
  ...
  toggle: async (id: string): Promise<Todo | undefined> => {
    // ⚡ Bolt: Atomic toggle using not() avoids initial select query
    const results = await db
      .update(todos)
      .set({
