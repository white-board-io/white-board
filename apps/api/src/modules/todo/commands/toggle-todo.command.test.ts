...
describe("toggleTodoHandler", () => {
  ...
  it("should return not found, when todo does not exist", async () => {
    ...
    if (!result.isSuccess) expect(result.errors?.[0]?.code).toBe("RESOURCE_NOT_FOUND");
  });
  ...
  it("should return validation errors, when id is invalid", async () => {
    ...
    expect(result.isSuccess).toBe(false);
    if (!result.isSuccess) expect(result.errors?.[0]?.code).toBe("INVALID_TODO_ID_FORMAT");
  });
  ...
  it("should toggle completion, when todo exists", async () => {
    ...
    if (result.isSuccess) expect(result.data).toEqual(updatedTodo);
  });
});
